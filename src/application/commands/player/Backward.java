...
/**
 * Class for Backwards command
 */
@Getter
public final class Backward extends PlayerRelatedCommands {
     ...
     * @param players
     */
    public Backward(final String command, final String username,
     ...
     * Backward command
     */
    public void backwardAction(final Player currentPlayer) {
        ...
        if (currentPlayer.getSituationPodcasts().get(i).getNameEpisode().equals(
                episode.getName())) {
            if (currentPlayer.getSituationPodcasts().get(i).getMinute() >=  NINETY) {
                currentPlayer.getSituationPodcasts().get(i).setMinute(
                        currentPlayer.getSituationPodcasts().get(i).getMinute() - NINETY);
            } else {
    ...
    public void startCommand(final ObjectMapper objectMapper, final ArrayNode outputs) {
        ...
        this.backwardAction(currentPlayer);
